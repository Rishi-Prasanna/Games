...
def results():
    ...
    teamBid = 0
    teamMade = 0
    TBP = 0 # Ten bag penalty.
    for P in TeamA:
        print(P.name + " | Made: " + str(P.numT) + ", Bidded: " + str(P.bidded))
        teamBid = teamBid + P.bidded
        teamMade = teamMade + P.numT
    # Points:
    # if tricks < bidded, then minus bidded * 10.
    # if tricks > bidded, then plus (bidded * 10) + (numT - bidded)
    # if numT - bidded >= 10, ten bag penalty.
    bags = teamMade - teamBid
    if bags < 0:
        bags = 0

    print("Total bid: " + str(teamBid))
    print("Total made: " + str(teamMade))
    if teamMade < teamBid:
        handPointsA = teamBid * -10
    else:
        handPointsA = (teamBid * 10)
    print("Hand points: " + str(handPointsA))
    while bags >= 10:
        TBP = TBP - 100
        bags = bags - 10
    print("Hand bags: " + str(bags))
    print("Ten bag penalty: " + str(TBP))
    ScoreA = handPointsA + bags + TBP
    print("Score for Team A: " + str(ScoreA))
    input("Press Enter to continue")
    ...
    print("Total made: " + str(teamMade))
    if teamMade < teamBid:
        handPointsB = teamBid * -10
    else:
        handPointsB = (teamBid * 10)
    print("Hand points: " + str(handPointsB))
    while bags >= 10:
        TBP = TBP - 100
        bags = bags - 10
    print("Hand bags: " + str(bags))
    print("Ten bag penalty: " + str(TBP))
    ScoreB = handPointsB + bags + TBP
    print("Score for Team B: " + str(ScoreB))
    input("Press Enter to continue")
    if ScoreA > ScoreB:
        youWin()
    elif ScoreB >= ScoreA:
